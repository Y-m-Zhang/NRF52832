...
//extern  bool LIS3DH_ReadReg(u8_t Reg, u8_t* Data);
//extern  u8_t LIS3DH_WriteReg(u8_t WriteAddr, u8_t Data);
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
uint8_t LIS3DH_Init(void)
{
    ...
    spi_config.ss_pin = SPI_CS_PIN; //nRF51822只能使用GPIO作为片选，所以这个单独定义了SPI CS管脚
